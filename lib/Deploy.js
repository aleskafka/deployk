	...
	if (remote = this.byFile.remoteList.getFirst(file)) {
		if (typeof remote[0] === 'string') {
			...
			if (remoteInst = this.remoteList.findByName(file, remote[0])) {
				return new RemoteWrapper(remoteInst.client, path.join(remoteInst.path, remote[1]||''), remote[2]);
			}
...
Deploy.prototype.loadFile = function(event, file)
{
	...
	var deployInst;
	try {
		deployInst = require(resolve.sync('deployk', {basedir: path.dirname(file)}));
		...
		this.emit('loadError', rfile, 'Local deployk package not found.');
	}
