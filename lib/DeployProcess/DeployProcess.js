			...
			if (opts.dry) {
				...
				self.remote.setup(opts.setup, function(err) {
					if (err && opts.setup) {
						...
						self.abort('Could not setup deploy dir. Project dir is probably missing, call setup.');
					}
 ...
 * @param {function}
 */
DeployProcess.prototype.runMirrorTree = function(cb)
{
	this.mirrorTree.run(cb);
}
